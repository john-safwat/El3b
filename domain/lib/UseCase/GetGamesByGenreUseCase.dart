...
GetGamesByGenreUseCase injectGetGamesByGenreUseCase(){
  return GetGamesByGenreUseCase(repository: injectRAWGGamesRepository());
}
...
class GetGamesByGenreUseCase {
  ...
  GetGamesByGenreUseCase({required this.repository});

  Future<(num? , List<RAWGGame>?)> invoke({required String genre , required int pageNumber , required String uid})async{
    var (count , response) = await repository.getGamesByGenre(genre: genre, pageNumber: pageNumber);
